...
int tm1637Init(gpio_port_t bClock, gpio_pin_t bClockP, gpio_port_t bData, gpio_pin_t bDataP)
{
	...
	bClockPin = bClockP;
	bDataPin = bDataP;
	//viv- GPIO_Config_Pin(bClockPort, bClockPin, PIN_MODE_OUTPUT_PP);
	//viv- GPIO_Config_Pin(bDataPort, bDataPin, PIN_MODE_OUTPUT_PP);
	//viv- GPIO_Set_Pin_Low(bClockPort, bClockPin);
	//viv- GPIO_Set_Pin_Low(bDataPort, bDataPin);
	return 0;
}
...
{
	//viv- GPIO_Set_Pin_High(bDataPort, bDataPin);
	//viv- GPIO_Set_Pin_High(bClockPort, bClockPin);
	usleep(CLOCK_DELAY);
	//viv- GPIO_Set_Pin_Low(bDataPort, bDataPin);
}
...
void TM1637_init() {
	...
	tm1637Init(GPIO_PORT_C, GPIO_PIN_4, GPIO_PORT_B, GPIO_PIN_4 | GPIO_PIN_5); 
	tm1637SetBrightness(4);
}   
	
void TM1637_process() {
	static int hours = 0, min = 0, secs = 0;
	...
	secs++;
	//viv- CLK_Delay_ms(1000);
	
	//viv- GPIO_Toggle_Pin(GPIO_PORT_B, GPIO_PIN_5);
}
